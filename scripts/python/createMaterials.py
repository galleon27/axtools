...
class UnifiedMaterialBuilder:
    def __init__(self, engine='octane'):
        """
        Initializes the unified builder. 
        :param engine: 'octane' or 'redshift'. Can be driven dynamically via HDA parameter.
        """
    ...
    def _setup_config(self):
        """Centralizes channel configurations and maps parameters dynamically based on the target renderer."""
        configs = {
            'basecolor': {'suffixes': self.node.parm('basecolor_suffix').eval().split(), 'dir_parm': 'basecolordir'},
            'ao': {'suffixes': self.node.parm('ambientocclusion_suffix').eval().split(), 'dir_parm': 'aodir'},
            'specular': {'suffixes': self.node.parm('specular_suffix').eval().split(), 'dir_parm': 'speculardir'},
            'roughness': {'suffixes': self.node.parm('roughness_suffix').eval().split(), 'dir_parm': 'roughnessdir'},
            'metallic': {'suffixes': self.node.parm('metallic_suffix').eval().split(), 'dir_parm': 'metallicdir'},
            'opacity': {'suffixes': self.node.parm('opacity_suffix').eval().split(), 'dir_parm': 'opacitydir'},
            'normal': {'suffixes': self.node.parm('normal_suffix').eval().split(), 'dir_parm': 'normaldir'},
            'displacement': {'suffixes': self.node.parm('displacement_suffix').eval().split(), 'dir_parm': 'displacementdir'},
            'emission': {'suffixes': self.node.parm('emission_suffix').eval().split(), 'dir_parm': 'emissivedir'}
        }

        if self.engine == 'octane':
            configs['basecolor'].update({'type': 'NT_TEX_IMAGE', 'color_space': 'NAMED_COLOR_SPACE_SRGB', 'input': 'baseColor'})
            configs['ao'].update({'type': 'NT_TEX_IMAGE', 'color_space': 'NAMED_COLOR_SPACE_OTHER'})
            configs['specular'].update({'type': 'NT_TEX_FLOATIMAGE', 'color_space': 'NAMED_COLOR_SPACE_OTHER', 'input': 'specular'})
            configs['roughness'].update({'type': 'NT_TEX_FLOATIMAGE', 'color_space': 'NAMED_COLOR_SPACE_OTHER', 'input': 'roughness'})
            configs['metallic'].update({'type': 'NT_TEX_FLOATIMAGE', 'color_space': 'NAMED_COLOR_SPACE_OTHER', 'input': 'metallic'})
            configs['opacity'].update({'type': 'NT_TEX_FLOATIMAGE', 'color_space': 'NAMED_COLOR_SPACE_OTHER', 'input': 'opacity'})
            configs['normal'].update({'type': 'NT_TEX_IMAGE', 'color_space': 'NAMED_COLOR_SPACE_OTHER', 'input': 'normal'})
            configs['displacement'].update({'type': 'NT_TEX_FLOATIMAGE', 'color_space': 'NAMED_COLOR_SPACE_OTHER', 'input': 'displacement'})
            configs['emission'].update({'type': 'NT_TEX_IMAGE', 'color_space': 'NAMED_COLOR_SPACE_SRGB', 'input': 'emission'})
            self.file_parm = 'A_FILENAME'
            self.cs_parm = 'colorSpace'

        elif self.engine == 'redshift':
            for k in configs: configs[k]['type'] = 'redshift::TextureSampler'
            configs['basecolor'].update({'color_space': 'Auto', 'input': 'base_color'})
            configs['ao'].update({'color_space': 'Auto'})
            configs['specular'].update({'color_space': 'Raw', 'input': 'refl_weight'})
            configs['roughness'].update({'color_space': 'Raw', 'input': 'refl_roughness'})
            configs['metallic'].update({'color_space': 'Raw', 'input': 'metalness'})
    ...
    def _scan_and_map_textures(self):
        """Builds a master dictionary mapping base names to their respective texture files in O(N) complexity."""
        inventory = {}
        suffix_to_channel = {}
    ...
    def get_or_create_material(self, name):
        """Retrieves existing material networks or creates them natively based on the active engine."""
        is_new = False
        ...
        if self.engine == 'octane':
            ...
            material_node, output_node = None, material.node('surface_output')
            ...
            material_node, output_node = None, None
            ...
            if not material_node: material_node = material.createNode('redshift::StandardMaterial')
            if not output_node: output_node = material.createNode('redshift_material')
                
            output_node.setNamedInput('Surface', material_node, 0)
            return material, material_node, output_node, is_new

    def _get_or_update_image_node(self, channel_key, material, name):
        """Internal helper utilizing engine-agnostic configuration parameters to update/create textures."""
        target_file = self.material_inventory.get(name, {}).get(channel_key)
        if not target_file: return None, None, False
        ...
        if existing_image:
            ...
            if "color_space" in properties:
                image.parm(self.cs_parm).set(properties["color_space"])
                
            if texdir_parm: texdir_parm.set(image.parm(self.file_parm))
            made_change = True

        return image, properties, made_change

    def setup_albedo_ao(self, material, material_node, name):
        """Handles combining Base Color and AO maps dynamically per engine."""
        ...
        base_color_input = self.channel_configs['basecolor']['input']
        ...
        if ao_img:
            ...
            if not mult_node:
                mult_node_type = 'NT_TEX_MULTIPLY' if self.engine == 'octane' else 'redshift::RSMathMulVector'
                mult_node = material.createNode(mult_node_type)
            ...
            material_node.setNamedInput(base_color_input, mult_node, 0)
        else:
            material_node.setNamedInput(base_color_input, albedo_img, 0)
            
        return made_change

    def create_texture_node(self, channel_key, material, target_node, name, secondary_node_type=None, secondary_input='texture', defaults=None):
        ...
        ch_input = self.channel_configs[channel_key]['input']
    ...
    def build(self):
        ...
        disp_amount = custom_disp_scale if custom_disp_scale is not None else 0.01
        ...
        for name in material_names:
            ...
            changes = [self.setup_albedo_ao(material, material_node, name)]
            
            if self.engine == 'octane':
                changes.extend([
                    self.create_texture_node('roughness', material, material_node, name),
                    self.create_texture_node('specular', material, material_node, name),
                    self.create_texture_node('metallic', material, material_node, name),
                    self.create_texture_node('normal', material, material_node, name),
                    self.create_texture_node('opacity', material, material_node, name),
                    self.create_texture_node('displacement', material, material_node, name, 
                                             secondary_node_type='NT_VERTEX_DISPLACEMENT', defaults={'black_level': 0.5, 'amount': disp_amount}),
                    self.create_texture_node('emission', material, material_node, name, 
                ...
                changes.extend([
                    ...
                    self.create_texture_node('emission', material, material_node, name),
                    self.create_texture_node('normal', material, material_node, name, 
                                             secondary_node_type='redshift::BumpMap', secondary_input='input', defaults={'inputType': '1'}),
                    self.create_texture_node('displacement', material, output_node, name, 
                                             secondary_node_type='redshift::Displacement', secondary_input='texMap', defaults={'scale': disp_amount})
                ])
        ...
        msg_lines = [f"Processed {total_materials} {self.engine.capitalize()} material(s)."]
...
def execute(engine=None):
    """
    Entry point. By default, checks for a 'renderer' parameter on the HDA, 
    otherwise falls back to an explicit argument or 'octane' default.
    """
    if not engine:
        try:
            renderer_parm = hou.pwd().parm('renderer')
            engine = renderer_parm.evalAsString().lower() if renderer_parm else 'octane'
        except Exception:
            engine = 'octane'
            
    builder = UnifiedMaterialBuilder(engine=engine)
    builder.build()